...
```{r}
#| label: load-data
buoy <- read_table('data.raw/buoy_2024.txt')
```
...
write_csv(buoy, file = 'data.raw/buoy_clean.csv')
```
...
buoy_clean <- read_csv('data.raw/buoy_clean.csv',
...
```{r}
write_csv(buoy_clean, file = 'data.clean/buoy_final.csv')
```
...
buoy_final |>
  ggplot(mapping = aes(x = mm,
                       y = atmp)) +
  geom_jitter(aes(colour = mm), 
              width = 0.1,
              alpha = 0.3) +
  geom_smooth(color = 'black')
```
> I could not for the LIFE of me figure out how to make the points rainbow, so I just stuck with the blue gradient. If it's a color or theme thing let me know because I would LOVE to know how to make it prettier and visually easier to read!
...
>This dataset looks at information about species in National Parks, with information such as species status, abundance, and whether or not the species is native. I could look at a few different research questions, including
"Which National Parks have the most invasive species?"
"Do NPs with more invasive species have more species with 'endangered' conservation statuses?"
"What is the relationship between observations of a particular species and conservation status?"
